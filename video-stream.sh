...
then
      ...
      ffmpeg -s $video_width\x$video_height -f video4linux2 -i $usb_camera -r $video_frame \
          -vf "drawtext = fontfile = /usr/share/fonts/truetype/ttf-dejavu/DejaVuSans-Bold.ttf: text ='$apparatus_location ($apparatus_name $apparatus_id) - %{localtime\:%X}': :fontsize=$font_size:fontcolor=black:box=1:boxcolor=white@0.5$
